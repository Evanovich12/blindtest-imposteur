...
async function fetchPlaylistTracks(id) {
  ...
  const tracks = trackList
    ...
    .map(t => ({ title: t.title, artist: t.subtitle || "", previewUrl: t.audioPreview.url, artwork: "" }));
  if (!tracks.length) throw new Error("Aucun extrait audio disponible dans cette playlist Spotify.");
  // Fetch per-track artwork from iTunes in parallel (batches of 5)
  for (let i = 0; i < tracks.length; i += 5) {
    const batch = tracks.slice(i, i + 5);
    const arts = await Promise.allSettled(batch.map(t => itunesArtwork(t.title, t.artist)));
    arts.forEach((res, j) => { if (res.status === "fulfilled" && res.value) tracks[i+j].artwork = res.value; });
  }
  return tracks;
}
...
function startRound(room) {
  ...
  io.to(room.code).emit("phase", { phase: "round", roundIndex: room.roundIndex, total: room.order.length });
}
...
io.on("connection", (socket) => {
  ...
  // Hôte signale qu'un nouveau morceau est en cours (auto-reset found + scores vitesse)
  socket.on("host:playTrack", ({ track }) => {
    const room = rooms[roomCode];
    if (!room) return;
    // Sauvegarder les données de streak du morceau précédent
    room.lastTrackFoundBoth = {};
    for (const [id, found] of Object.entries(room.trackFound || {})) {
      room.lastTrackFoundBoth[id] = found.title && found.artist;
    }
  ...
  socket.on("player:guess", ({ text }) => {
    ...
    // Streak : vient de trouver les 2
    if (found.title && found.artist && (titleHit || artistHit)) {
      const hadBothBefore = !titleHit && !artistHit; // déjà complet avant ce guess ?
      if (!hadBothBefore) {
        if (room.lastTrackFoundBoth?.[pid]) {
          room.streaks[pid] = (room.streaks[pid] || 1) + 1;
          const streakBonus = Math.min(room.streaks[pid] - 1, 5) * 2;
          room.players[pid].score += streakBonus; pointsEarned += streakBonus;
        } else {
          room.streaks[pid] = 1;
        }
      }
