...
app.get("/debug/spotify-token", async (req, res) => {
  ...
  try {
    ...
    const r = await fetch(embedUrl, { headers: { "User-Agent": "Mozilla/5.0", "Accept": "text/html" } });
    log.push(`Embed fetch: ${r.status}`);
    const html = await r.text();
    const m = html.match(/<script id="__NEXT_DATA__"[^>]*>(\{.*?\})<\/script>/s);
    if (!m) { log.push("__NEXT_DATA__ introuvable"); return res.json({ log }); }
    const data = JSON.parse(m[1]);
    const token = data?.props?.pageProps?.state?.settings?.session?.accessToken;
    log.push(`Token: ${token ? token.substring(0, 20) + "..." : "ABSENT"}`);
    const trackList = data?.props?.pageProps?.state?.data?.entity?.trackList;
    log.push(`TrackList embed: ${trackList?.length ?? 0} tracks`);
    if (token) {
      ...
      log.push(`API offset=100 items: ${apiData.items?.length ?? 0}, sample: ${JSON.stringify(apiData.items?.slice(0,2).map(i=>i?.track?.name))}`);
      if (apiData.error) log.push(`API error: ${JSON.stringify(apiData.error)}`);
    }
