...
async function fetchPlaylistTracks(id) {
  ...
  const accessToken = data?.props?.pageProps?.accessToken;
  ...
  const totalTracks = entity?.totalTrackCount || trackList.length;
  if (accessToken && totalTracks > trackList.length) {
    const limit = 50;
    for (let offset = trackList.length; offset < Math.min(totalTracks, 800); offset += limit) {
      try {
  ...
  const seen = new Set();
  const tracks = allItems.filter(t =>
    t.title && t.previewUrl && !seen.has(t.previewUrl) && seen.add(t.previewUrl)
  );
