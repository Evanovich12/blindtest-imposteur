...
async function fetchPlaylistTracks(id) {
  ...
  if (trackList.length >= 100) {
    // Priority: 1) client_credentials (app token) 2) web-player anonymous token 3) embed token
    ...
    try {
      ...
      if (wpR.ok) { const wpD = await wpR.json(); webPlayerToken = wpD.accessToken || null; }
    } catch (_) {}
    const apiToken = (await getSpotifyToken()) || webPlayerToken || embedToken;
    if (apiToken) {
      const limit = 50;
