...
app.get("/debug/spotify-token", async (req, res) => {
  const id = req.query.id || "1wgDRqCdpmAfjrYPJGEvhH";
  const log = [];
  try {
    // Test client_credentials token first
    const clientId = process.env.SPOTIFY_CLIENT_ID;
    const clientSecret = process.env.SPOTIFY_CLIENT_SECRET;
    log.push(`Env SPOTIFY_CLIENT_ID: ${clientId ? clientId.substring(0,8)+"..." : "ABSENT"}`);
    log.push(`Env SPOTIFY_CLIENT_SECRET: ${clientSecret ? "présent" : "ABSENT"}`);
    const ccToken = await getSpotifyToken();
    log.push(`Client-credentials token: ${ccToken ? ccToken.substring(0,20)+"..." : "ÉCHEC"}`);
    // Also test web-player anonymous token
    try {
      const wpR = await fetch("https://open.spotify.com/get_access_token?reason=transport&productType=web_player", {
        headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", "Accept": "application/json" },
      });
      const wpD = await wpR.json();
      log.push(`Web-player token: ${wpD.accessToken ? wpD.accessToken.substring(0,20)+"... (isAnonymous="+wpD.isAnonymous+")" : "ÉCHEC"}`);
      if (wpD.accessToken && !ccToken) {
        const t2 = await fetch(`https://api.spotify.com/v1/playlists/${id}/tracks?limit=5&offset=100&fields=items(track(name))`, { headers: { "Authorization": `Bearer ${wpD.accessToken}` } });
        log.push(`Web-player API offset=100 status: ${t2.status}`);
        const d2 = await t2.json();
        log.push(`Titres web-player offset=100: ${JSON.stringify((d2.items||[]).slice(0,3).map(i=>i?.track?.name))}`);
      }
    } catch (e2) { log.push(`Web-player exception: ${e2.message}`); }
    if (ccToken) {
      const apiR = await fetch(
        `https://api.spotify.com/v1/playlists/${id}/tracks?limit=5&offset=100&fields=items(track(name))`,
        { headers: { "Authorization": `Bearer ${ccToken}` } }
      );
      log.push(`API offset=100 status: ${apiR.status}`);
      if (apiR.status === 429) log.push(`Retry-After: ${apiR.headers.get("Retry-After")}`);
      const apiData = await apiR.json();
      log.push(`Titres offset=100: ${JSON.stringify((apiData.items||[]).slice(0,3).map(i=>i?.track?.name))}`);
      if (apiData.error) log.push(`Erreur API: ${JSON.stringify(apiData.error)}`);
    }
...
async function fetchPlaylistTracks(id, clientToken) {
  ...
  const embedToken = data?.props?.pageProps?.state?.settings?.session?.accessToken;
  ...
  if (trackList.length >= 100) {
    // Priority: 1) client browser token (has user session) 2) client_credentials 3) embed token
    const apiToken = clientToken || (await getSpotifyToken()) || embedToken;
    if (apiToken) {
      const limit = 50;
      for (let offset = 100; offset < 800; offset += limit) {
        ...
        while (retries < 4) {
          try {
            ...
            if (apiR.status === 429) {
              const retryAfter = parseInt(apiR.headers.get("Retry-After") || "2", 10);
        ...
        if (!apiR || !apiR.ok) break;
        let apiData;
        try { apiData = await apiR.json(); } catch (_) { break; }
...
async function importPlaylist(url, clientToken) {
  ...
  const all = await fetchPlaylistTracks(id, clientToken);
...
io.on("connection", (socket) => {
  ...
  socket.on("player:submitPlaylist", async ({ url, spotifyToken }, ack) => {
    try {
      const room = rooms[roomCode];
      if (!room || !pid) return ack && ack({ ok: false, error: "Salon fermé." });
      const tracks = await importPlaylist(url, spotifyToken);
