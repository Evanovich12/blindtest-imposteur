...
async function fetchPlaylistTracks(id, token) {
  ...
  while (url && out.length < 50) {
    ...
    if (r.status === 403) {
      let detail = "";
      try { const body = await r.json(); detail = JSON.stringify(body); } catch (_) {}
      console.error("Spotify 403 detail:", detail);
...
async function importPlaylist(url, userToken = null) {
...
io.on("connection", (socket) => {
  ...
  socket.on("player:submitPlaylist", async ({ url, userToken }, ack) => {
    const room = rooms[roomCode];
    if (!room || !pid) return ack && ack({ ok: false, error: "Salon ferme." });
    try {
      const tracks = await importPlaylist(url, userToken || null);
